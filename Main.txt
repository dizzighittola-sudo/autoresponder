...
function loadResources() {
  // ✅ FIX: Usa LockService per prevenire race condition tra trigger
  if (GLOBAL_CACHE.loaded) {
    console.log('📦 Resources already loaded (cached)');
    return;
  }
  
  const lock = LockService.getScriptLock();
  try {
    // Attendi max 5 secondi per il lock
    if (!lock.tryLock(5000)) {
      console.log('📦 Another process is loading resources, waiting...');
    ...
    try {
      ...
      // Lock potrebbe essere già rilasciato o non acquisito
    }
  }
}
