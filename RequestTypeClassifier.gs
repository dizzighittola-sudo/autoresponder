// ====================================================================
// REQUEST TYPE CLASSIFIER - Classificazione Tecnica/Pastorale/Mista
// Versione completa con indicatori ponderati
// ====================================================================

/**
 * RequestTypeClassifier - Classificatore tipo richieste
 * 
 ...
 * - AI-Core Lite: Solo quando needsDiscernment || needsDoctrine (tono, limiti)
 * - AI-Core: Solo quando needsDiscernment = true
 * - Dottrina: Solo quando needsDoctrine = true
 * (Per richieste puramente tecniche, nessuna KB viene caricata)
 */
class RequestTypeClassifier {
  constructor() {
    ...
    this.TECHNICAL_INDICATORS = [
      ...
      // Riferimenti a ruoli formali (weight 1-2)
    ...
    this.PASTORAL_INDICATORS = [
      ...
      // Situazioni di vita complesse - ITALIANO (weight 2)
      ...
      // Situazioni di vita complesse - ENGLISH (weight 2)
  ...
  classify(subject, body, externalHint = null) {
    ...
    // Determina flag di attivazione (sincronizzati con tipo richiesta)
    ...
    console.log(`   📊 Request classification: ${requestType.toUpperCase()} (Source: ${source})`);
    if (source === 'regex') {
      console.log(`      Tech=${technicalScore}, Pastor=${pastoralScore}, Doctr=${doctrineScore}`);
    }
    console.log(`      Discernment=${needsDiscernment}, Doctrine=${needsDoctrine}`);
