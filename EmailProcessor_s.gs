...
class EmailProcessor {
  ...
  processThread(thread, knowledgeBase, doctrineBase, labeledMessageIds = new Set(), skipLock = false) {
    ...
    if (skipLock) {
      ...
      lockValue = Date.now().toString(); // Identificativo unico per questo processo
      ...
      if (existingLock) {
        ...
        if (isStale) {
          console.warn(`🔓 Stale lock detected for thread ${threadId}, clearing`);
...
const promptContext = createPromptContext({
  ...
  memory: {
    ...
    // ✅ FIX Bug #6: Validate lastUpdated to prevent NaN/Crash
    lastUpdated: memoryContext.lastUpdated 
      ? (isNaN(new Date(memoryContext.lastUpdated).getTime()) 
          ? null 
      ...
      if (lockAcquired && scriptCache && threadLockKey) {
        try {
          ...
          if (!currentLockValue || currentLockValue === lockValue) {
            ...
            console.warn(`⚠️ Skipped lock release for thread ${threadId} (lock owned by another process)`);
          }
        } catch (e) {
  ...
  processUnreadEmails(knowledgeBase, doctrineBase = '') {
    ...
    threads.forEach((thread, index) => {
      ...
      // Per un sistema con volumi più alti, considerare lock document-based (Sheet row lock).
          // Processa il thread (Il lock è ora gestito internamente a processThread)
          // Rimosso parametro skipLock=true
...
function computeSalutationMode({ isReply, messageCount, memoryExists, lastUpdated, now = new Date() }) {
  ...
  if (isReply || messageCount > 1 || memoryExists) {
    ...
    // ✅ FIX ISSUE #5: Se timestamp invalido (NaN), tratta come mancante -> none_or_continuity
    // Unifica il comportamento tra null e dato corrotto per coerenza "active listening"
