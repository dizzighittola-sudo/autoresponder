...
function loadResources() {
  ...
  try {
    ...
    // ✅ Inizializza a array vuoto per evitare undefined (Fix BUG #1)
      ...
      try {
        ...
        if (controlSheet) {
          ...
          for (const row of ferieRows) {
            ...
            // ✅ Fix BUG #7: Parsing robusto e validazione logica
            ...
            try {
              ...
              if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                console.warn(`⚠️ Formato data non valido in riga ferie: ${row[1]} - ${row[2]}`);
                continue;
              }
              ...
              console.warn(`⚠️ Errore nel parsing delle date ferie: ${parsingErr.message}`);
              continue;
            }
          }
...
function main() {
  ...
  // Riduce il numero di email processate per run se ci stiamo avvicinando ai limiti giornalieri.
  if (CONFIG.USE_RATE_LIMITER) {
    try {
      ...
      if (maxPercent > 80) {
        ...
        // Riduce il limite della metà (minimo 1 email)
        CONFIG.MAX_EMAILS_PER_RUN = Math.max(1, Math.floor(originalLimit / 2));
        console.warn(`🚨 SAFETY VALVE ATTIVA (Quota RPD max: ${maxPercent}%): Ridotto MAX_EMAILS_PER_RUN da ${originalLimit} a ${CONFIG.MAX_EMAILS_PER_RUN}`);
      }
    } catch (e) {
      console.warn(`⚠️ Errore nel calcolo Safety Valve: ${e.message}`);
    }
  }
